...
class RLInterface:
    ...
    def parse_args(self):
        parser = argparse.ArgumentParser(description="Safe reinforcement learning interface")
        parser.add_argument("--grid-model", "-m", type=str, default="evade", help="Model of environment")
        parser.add_argument("--learning_method", type=str, default="SAC", help="Learning method")
    ...
    def create_model(self, strategy="SAC", num_episodes=10, model_type="evade"):
        if model_type == "evade":
            ...
            RLInterface.add_to_sys_argv_intercept()
        elif model_type == "refuel":
            RLInterface.add_to_sys_argv_refuel()
        elif model_type == "obstacle":
            RLInterface.add_to_sys_argv_obstacle()
        elif model_type == "avoid":
            RLInterface.add_to_sys_argv_avoid()
        elif model_type == "rocks":
            RLInterface.add_to_sys_argv_rocks()
        else:
            logger.error(f"Model type {model_type} not supported.")
            return
        RLInterface.add_to_sys_argv_strategy(model_type)
        RLInterface.add_to_sys_argv_num_episodes(10)
        print(sys.argv)
...
if __name__ == "__main__":
    ...
    sys.argv = clean_argv
    interface.create_model(args.learning_method, args.eval_episodes, args.model)
