...
import tf_agents
from safe_rl.deep_policy import DeepAgent, ReplayMemory
import re
import tf_agents

from safe_rl.model_simulator import SimulationExecutor
import logging
...
def compute_avg_return(env, agent, policy, num_episodes=10, max_steps=100):
    ...
    return_set = avg_return.numpy(), np.quantile(episode_rewards, 0.3173), np.quantile(episode_rewards, 0.6827), np.clip(avg_return.numpy(
    )-np.std(episode_rewards), min_value, env.goal_value), np.clip(avg_return.numpy()+np.std(episode_rewards), min_value, env.goal_value)
...
def record_track(recorder, executor, agent, policy, maxsteps, no_tracks=1):
    for _ in range(no_tracks):
        ...
        for n in range(maxsteps):
            ...
            executor._simulator.step(action)
            state = executor._simulator._report_state()
            executor._shield.track(
                action, executor._model.get_observation(state))
            assert state in executor._shield.list_support()
            recorder.record_available_actions(actions)
            recorder.record_allowed_actions(safe_actions)
            recorder.record_selected_action(action)
...
class TF_Environment(SimulationExecutor):
    def __init__(self, model, shield, obs_length=1, valuations=False, obs_type='BELIEF_SUPPORT', maxsteps=100, goal_value=1000):
        ...
        self.disc_spec = tf_agents.specs.BoundedTensorSpec(
            name='discount', dtype='float32', minimum=0, maximum=1, shape=tf.TensorShape(()))
        self.obs_spec = {'obs': tf_agents.specs.TensorSpec(name='observation', dtype='int32', shape=tf.TensorShape(
            obs_shape)), 'mask': tf_agents.specs.TensorSpec(shape=(self.nr_actions,), dtype='bool', name='mask')}
        self.rew_spec = tf_agents.specs.TensorSpec(
            name='reward', dtype='float32', shape=tf.TensorShape(()))
        self.step_spec = tf_agents.specs.TensorSpec(
            name='step_type', dtype='int32', shape=tf.TensorShape(()))
    ...
    def current_time_step(self, rew=None):
        if rew is not None:
            ...
            r = tf.constant([self._simulator._report_rewards()[0]]) if len(
                self._simulator._report_rewards()) != 0 else tf.constant([0.])
        ...
        if self.shield_on:
            ...
            if self.shield_switch_type == 'HARD':
                ...
                if np.random.random(1)[0] > self.decay:
                    safe_actions = self._shield.shielded_actions(
                        range(len(actions)))  # Shield on
                else:
                    safe_actions = actions
        ...
        if len(self._model.choice_labeling.get_labels_of_choice(self._model.get_choice_index(self._simulator._report_state(), 0))) == 0:
            ...
            mask_inds = [self.action_indices[self._model.choice_labeling.get_labels_of_choice(
                self._model.get_choice_index(self._simulator._report_state(), a_i)).pop()] for a_i in safe_actions]
        ...
        observation = {'obs': tf.constant(
            [self.observe()], dtype='int32'), 'mask': mask}
        if self.first:
            self.first = False
    ...
    def simulate_deep_RL(self, recorder, total_nr_runs=5, eval_interval=1000, eval_episodes=10, eval_env=None, agent_arg='DQN', hyper_param=None):
        ...
        if hyper_param:
            if 'network_size' in hyper_param:
                RL_agent = DeepAgent(self, alpha, agent_arg,
                                     layer_size=hyper_param['network_size'])
            else:
        ...
        avg_return, episodes = compute_avg_return(
            eval_env, RL_agent.agent, RL_agent.agent.policy, num_eval_episodes, max_steps=self.maxsteps)
        # record_track(recorder, eval_env, RL_agent.agent, RL_agent.agent.policy, maxsteps)
        self.reset()
        collect_data(self, RL_agent.agent,
                     RL_agent.agent.collect_policy, buffer, steps=2)
        dataset = buffer.as_dataset(
            num_parallel_calls=1, sample_batch_size=64, num_steps=2).prefetch(3)
        iterator = iter(dataset)
        RL_agent.agent.train = common.function(RL_agent.agent.train)
        returns = [(0,)+avg_return]
        episodes_list = [[0]+episodes]
        rand_pol = tf_agents.policies.random_tf_policy.RandomTFPolicy(
            self.time_step_spec, self.act_spec, observation_and_action_constraint_splitter=RL_agent.observation_and_action_constraint_splitter)
        print(
        ...
        for _ in range(total_nr_runs):
            ...
            if agent_arg == 'REINFORCE':
                collect_episode(self, RL_agent.agent.collect_policy,
                                collect_steps_per_iteration, buffer)
                experience = buffer.gather_all()
                train_loss = RL_agent.agent.train(experience).loss
                buffer.clear()
            else:

                collect_data(self, RL_agent.agent,  RL_agent.agent.collect_policy,
                             buffer, collect_steps_per_iteration)
                experience, unused_info = next(iterator)
                train_loss = RL_agent.agent.train(experience).loss

            step = int(RL_agent.agent.train_step_counter.numpy(
            )/25) if agent_arg == 'PPO' else RL_agent.agent.train_step_counter.numpy()
    ...
    def get_observation_keywords(self):
        if self.obs_type == 'STATE_LEVEL':
            keywords = list(
                [i[1:-2] for i in str(self._model.state_valuations.get_json(0)).split()[1:-1:2]])
        else:
            keywords = list(
                [i[1:-2] for i in str(self._model.observation_valuations.get_json(0)).split()[1:-1:2]])
        return keywords
    ...
    def collect_buffer(self, total_nr_runs=5, total_nr_good_runs=1):
        ...
        for _ in range(total_nr_runs):
            if agent_arg == 'REINFORCE':
                good_run = collect_episode(
                    self, rand_pol, collect_steps_per_iteration, buffer)
                if good_run:
                    experience = buffer.gather_all()
