...
def compute_avg_return(env, agent, policy, num_episodes=10,max_steps=100):
    ...
    for _ in range(num_episodes):
        ...
        while not time_step.is_last():
            ...
            safe_actions = env._shield.shielded_actions(range(len(actions))) # Shieldování
            action_step = policy.action(time_step) # Výběr nejlepší akce dle aktuální naučené politiky
            time_step = env.step(env.apply_action(action_step.action)) # Vykonání kroku v prostředí a pamatování si aktuálního stavu
...
class TF_Environment(SimulationExecutor):
    ...
    def simulate_deep_RL(self, recorder, total_nr_runs=5,eval_interval=1000,eval_episodes=10,eval_env=None,agent_arg='DQN',hyper_param=None):
        ...
        compute_avg_return(eval_env,RL_agent.agent, RL_agent.agent.policy, 50, max_steps=self.maxsteps)
