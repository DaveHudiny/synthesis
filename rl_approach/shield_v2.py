...
from safe_rl.model_simulator import Tracker
from safe_rl.recorder import LoggingRecorder, VideoRecorder, StatsRecorder
from safe_rl.noshield import NoShield
import argparse
import copy

import safe_rl.deep_policy
...
def compute_winning_region(model, formula, initial=True):
    ...
    model = sp.pomdp.prepare_pomdp_for_qualitative_search_Double(
        model, formula)
    solver = sp.pomdp.create_iterative_qualitative_search_solver_Double(
...
def parse_args(cfg=None):
    if cfg:
        ...
        parser.add_argument(
            '--seed', help="Seed for randomised movements", default=3)
        ...
        model_group.add_argument(
            '--grid-model', '-m', help=f'Model from the gridworld-by-storm visualisation set, choose from {str(experiment_names)}')
        model_group.add_argument(
            '--prism', help="Specify model from prism file")
        parser.add_argument('--prop', help='Specify property string directly')
        parser.add_argument(
            '--constants', '-c', help="Constants to select the instance of the model", default="")
        parser.add_argument('--load-winning-region', '-wr',
                            help="Load a winning region")
        parser.add_argument(
        ...
        parser.add_argument('--goal-value', type=int, default=100)
...
def improved_main(cfg=None):
    ...
    if args.prism and not args.prop:
        raise RuntimeError(
            "Prism models require setting the property via --prop")
    if args.grid_model and args.prop:
        raise RuntimeError(
            "Properties cannot be set manually when using the gridstorm models")
    ...
    if args.grid_model:
        ...
        if args.constants is None and len(model_constants) > 0:
            raise RuntimeError("Model constants {} defined, but not given by command line".format(
                ",".join(model_constants)))
    ...
    if args.load_winning_region:
        ...
        winning_region, preamble = stormpy.pomdp.BeliefSupportWinningRegion.load_from_file(
            args.load_winning_region)
    ...
    if hasattr(args, 'network_size'):
        hyper_param.update(
            {'network_size': args.__getattribute__('network_size')})
    ...
    learn_executor = TF_Environment(learn_model, tracker, obs_length=1, maxsteps=args.maxsteps,
                                    obs_type=obs_type, valuations=valuations, goal_value=args.goal_value)
    eval_executor = TF_Environment(eval_model, tracker, obs_length=1, maxsteps=args.maxsteps,
                                   obs_type=obs_type, valuations=valuations, goal_value=args.goal_value)
    if hasattr(args, 'switch_shield'):
        learn_executor.set_shield_switch(
            args.shield_episode, args.switch_shield)
    ...
    print("Starting RL:\n")
    G0, episodes, model = learn_executor.simulate_deep_RL(recorder=None, total_nr_runs=args.max_runs, eval_interval=args.eval_interval,
