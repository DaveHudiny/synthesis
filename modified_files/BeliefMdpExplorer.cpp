...
// get probabilityEstimation from Storage
template<typename PomdpType, typename BeliefValueType>
typename BeliefMdpExplorer<PomdpType, BeliefValueType>::ValueType BeliefMdpExplorer<PomdpType, BeliefValueType>::getProbabilityEstimation(
    MdpStateType const &mdpState) const {
    STORM_LOG_ASSERT(status == Status::Exploring, "Method call is invalid in current status.");
    return probabilityEstimation[mdpState];
}
