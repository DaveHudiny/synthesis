...
class ActorDistributionRnnNetwork(network.DistributionNetwork):
  ...
  def __init__(
    ...
    Args:
      ...
      observation_and_action_constraint_splitter: A callable that takes
        observations and returns (observations, mask), where mask is a
        tensor of shape outer_rank + [num_actions], with values in {0, 1}.
        Splitting observations in this way allows for action-conditional
        observation pre-processing. For example, the mask is used to mark
        which batch elements are valid, and the observations are used in
        the actor. The network will then be called with only the valid
        batch elements, and any invalid elements will be masked. The
        network should handle this masking in its internal preprocessing
        layers.
    ...
    if observation_and_action_constraint_splitter is not None:
      ...
      self._observation_and_action_constraint_splitter = lambda x: (x, None)
