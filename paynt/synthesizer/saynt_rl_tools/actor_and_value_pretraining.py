...
class Actor_Value_Pretrainer:
    def __init__(self, environment :Environment_Wrapper, tf_environment : TFPyEnvironment, args : ArgsEmulator, collect_data_spec = None):
        self.args = args

        self.environment = environment
        self.tf_environment = tf_environment

        self.actor_net = create_recurrent_actor_net_demasked(tf_environment, tf_environment.action_spec())
        self.critic_net = create_recurrent_value_net_demasked(tf_environment)

        self.actor_optimizer = tf.keras.optimizers.Adam(learning_rate=0.001, weight_decay=0.001)
        self.actor_loss_fn = tf.keras.losses.SparseCategoricalCrossentropy(from_logits=True)
        self.gamma = 0.99

        self.critic_optimizer = tf.keras.optimizers.Adam(learning_rate=0.001, weight_decay=0.001)
        self.critic_loss_fn = tf.keras.losses.MeanSquaredError()
        self.init_replay_buffer(tf_environment, collect_data_spec = collect_data_spec)
    ...
    def init_replay_buffer(self, tf_environment : TFPyEnvironment, collect_data_spec = None, buffer_size=None):
        ...
        modified_collect_data_spec = collect_data_spec._replace(policy_info={"mem_node": TensorSpec(shape=(), dtype=tf.int32, name='mem_node')})
    ...
    def init_fsc_policy_driver(self, tf_environment: TFPyEnvironment, fsc: FSC = None, info_spec = None):
        ...
        self.fsc_policy = FSC_Policy(tf_environment=tf_environment, fsc=fsc,
                                     ...
                                     info_spec={"mem_node": TensorSpec(shape=(), dtype=tf.int32, name='mem_node')},
                                     info_mem_node=True)
    ...
    def train_actor_iteration(self, actor_net : ActorDistributionRnnNetwork, experience):
        ...
        with tf.GradientTape() as tape:
                predicted_actions, _ = actor_net(observations, step_type = step_types)
        ...
        self.actor_optimizer.apply_gradients(zip(grads, actor_net.trainable_variables))
    ...
    def train_value_iteration(self, critic_net : ValueRnnNetwork, experience):
        ...
        rewards = rewards[:, :-1]
        predicted_values, _ = critic_net(current_observations, step_type = step_types[:, :-1])
        predicted_next_values, _ = critic_net(next_observations, step_type = step_types[:, 1:])
        returns = rewards + self.gamma * tf.stop_gradient(predicted_next_values)
        ...
        self.critic_optimizer.apply_gradients(zip(grads, critic_net.trainable_variables))
    ...
    def reinit_fsc_policy_driver(self, fsc : FSC = None):
        self.init_fsc_policy_driver(tf_environment=self.tf_environment, fsc=fsc)
    ...
    def train_both_networks(self, num_epochs : int, fsc : FSC, external_actor_net : ActorDistributionRnnNetwork = None, external_critic_net : ValueRnnNetwork = None):
        ...
        for epoch in range(num_epochs):
            ...
            actor_loss = self.train_actor_iteration(actor_net, experience=experience)
            critic_loss = self.train_value_iteration(critic_net, experience=experience)
    ...
    def get_duplex_driver(self, fsc : FSC, rl_agent : TFAgent, replay_buffer_fsc : TFUniformReplayBuffer, replay_buffer_rl : TFUniformReplayBuffer,
        ...
        observer = self.get_duplex_observer(replay_buffer_fsc=replay_buffer_fsc, replay_buffer_rl=replay_buffer_rl)
        ...
        duplex_driver = DynamicEpisodeDriver(self.tf_environment, eager, observers = [observer], num_episodes=1)
    ...
    def evaluate_actor(self, actor_net):
        pass
