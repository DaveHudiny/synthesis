...
class PredicateAutomata:
    def __init__(self, states : list[int], state_labels : list[str], transition_matrix : list[list[int]], predicates_set_labels : list[str], initial_state = 0,
        ...
        Args:
            states (list): List of states in the automata.
            transition_matrix (list): Transition matrix defining state transitions.
            predicates (list): List of predicates associated with each state.
            reward_function (function): Function to compute rewards based on states and actions.
        """
        self.states = states
    ...
    def _convert_binary_predicates_to_action_number(self, predicates : tf.Tensor) -> tf.Tensor:
        ...
        Args:
            predicates (tf.Tensor): Batch of lists of boolean predicates.

        Returns:
            tf.Tensor: Int representation of the predicates.
        """
    ...
    def evaluate_observation(self, observation) -> list[bool]:
        """Returns the predicates for the given observation.
        Args:
            observation (list): Observation to evaluate.
        
        Returns: 
            list: List of boolean valuations of predicates.
        """
    ...
    @tf.function
    def step(self, current_state, observation):
...
def create_dummy_predicate_automata(observation_labels : list[str] = None) -> PredicateAutomata:
    ...
    predicate_labels = ["amdone", "fuel=5"]
    ...
    automata = PredicateAutomata(states = states, state_labels = state_labels, transition_matrix = transition_matrix, predicates_set_labels = predicate_labels,
                                 observation_labels=observation_labels, predicate_based_rewards=True, initial_state=0)
    return automata
