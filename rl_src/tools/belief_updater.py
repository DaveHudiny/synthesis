...
class Belief_Updater:
    ...
    def init_observation_action_lengths(self):
        self.observation_action_lengths = np.zeros(
            (self.nr_observations,), dtype=np.int32)
        for obs in range(len(self.action_labels_at_observation)):
            self.observation_action_lengths[obs] = len(
                self.action_labels_at_observation[obs])
        self.max_number_of_actions = np.max(self.observation_action_lengths)
        self.observation_action_lengths = tf.convert_to_tensor(
            self.observation_action_lengths, dtype=tf.int32)

    def init_observation_state_mapping(self):
        observation_state_mapping = np.zeros(
            (self.nr_observations, self.nr_states))
        observation_mask = self.pomdp_observations == np.arange(self.nr_observations)[
            :, None]
        observation_state_mapping[observation_mask] = 1
        observation_state_mapping /= np.sum(
            observation_state_mapping, axis=1, keepdims=True)
        self.observation_state_mapping = tf.convert_to_tensor(
            observation_state_mapping, dtype=tf.float32)

    def init_choice_index_matrix(self, max_number_of_actions):
        choice_index_matrix = np.zeros(
            (self.nr_states, max_number_of_actions), dtype=np.int32)
        for state in range(self.nr_states):
            for action in range(max_number_of_actions):
                ...
                if choice >= self.nr_choices:
                    continue
                choice_index_matrix[state, action] = self.get_choice_index(
                    state, action)
        self.choice_index_matrix = tf.convert_to_tensor(
            choice_index_matrix, dtype=tf.int32)

    def init_transition_matrix(self):
        choice_transition_matrix = np.zeros(
            (self.nr_choices, self.nr_states,), dtype=np.float32)
        ...
        self.transition_matrix = tf.convert_to_tensor(
            choice_transition_matrix, dtype=tf.float32)
    ...
    def next_belief(self, belief, action_label, next_obs):
        ...
        new_belief = {state: prob/prob_sum for state,
                      prob in new_belief.items()}
        return new_belief
    ...
    def nullify_illegal_states(self, belief, next_obs):
        mask = tf.cast(
            self.observation_state_mapping[next_obs] > 0.0, tf.float32)
        belief = belief * mask
        return belief

    def update_belief(self, belief, obs, next_obs):  # Update single belief state
        choices = self.choice_index_matrix[:,
                                           :self.observation_action_lengths[obs]]
        ...
        # Zero out all states that do not emit next_obs
        new_belief = self.nullify_illegal_states(new_belief, next_obs)
        belief_sum = tf.reduce_sum(new_belief)
        new_belief = tf.cond(belief_sum > 0, lambda: new_belief /
                             belief_sum, lambda: new_belief)  # Normalize only if sum > 0
        return new_belief
        ...
        if len(observations.shape) > 1:
            for i in range(len(observations)):  # Iterate over batches
                ...
                for j in range(len(observations[i]) - 1):
                    sub_belief = self.update_belief(
                        sub_belief, observations[i, j], observations[i, j+1])
                    sub_beliefs.append(sub_belief)
                beliefs.append(sub_beliefs)
        else:
            obs_pairs = tf.stack([observations[:-1], observations[1:]], axis=1)
            beliefs = tf.scan(self.update_belief_paired,
                              obs_pairs, initializer=belief)
