...
class PPOAgent(tf_agent.TFAgent):
  ...
  def __init__(
    ...
    Args:
      ...
      log_prob_loss: A float to indicate the log probability loss. By default set
        to 0.0, which means that the log probability loss is not used. If set to
        a positive value, the log probability loss will be added to the total
        loss. This should motivate agent to add at least small probability to
        all actions.
  ...
  def get_loss(
    ...
    if self.log_prob_loss > 0.0:
      log_prob_loss = compute_log_prob_loss(
          act_log_probs, weights=weights
      ) * self.log_prob_loss
    else:
      log_prob_loss = tf.zeros_like(policy_gradient_loss)
