...
@gin.configurable
class DynamicRewardDriver(driver.Driver):
  ...
  This termination condition can be overridden in subclasses by implemaenting the
  ...
  def __init__(
      ...
      num_steps=1,
      trajectory_reward_generator : callable = None,
  ):
    ...
    Raises:
      ValueError:
        ...
        tf_policy.TFPolicy.
    """
    super(DynamicRewardDriver, self).__init__(
    ...
    self._get_curiosity_reward = trajectory_reward_generator
  ...
  def _loop_body_fn(self):
    ...
    def loop_body(counter, time_step, policy_state):
      ...
      curiosity_reward = self._get_curiosity_reward(
            old_policy_state, policy_state, next_time_step.step_type
        )
      # Compute the curiosity reward by oriiginal reward + new curiosity reward
      curiosity_reward = traj.reward + curiosity_reward
      traj = traj._replace(
