...
class NeuralFSCActor(models.Model):
    def __init__(self, observation_shape, action_range, memory_len):
        ...
        self.simple_rnn_for_memory = layers.SimpleRNN(memory_len, return_sequences=True, return_state=True)
    ...
    def call(self, inputs, step_type: StepType, old_memory = None):
        if step_type.shape[-1] == 0:
            step_type = tf.zeros((step_type.shape[0], step_type.shape[1], 1))
        else:
            step_type = tf.reshape(step_type, (step_type.shape[0], step_type.shape[1], 1))
...
class ClonedFSCActorPolicy(TFPolicy):
    def __init__(self, original_policy: TFPolicy, memory_size: int, observation_and_action_constrint_splitter=None):
        ...
        policy_state_spec = BoundedArraySpec(
            (memory_size,), np.float32, minimum=0, maximum=2)
    ...
    def distro(self, time_step, policy_state, seed):
        ...
        step_type = tf.reshape(time_step.step_type, (time_step.step_type.shape[0], 1, -1))
...
def sample_data_with_policy(policy: TFPolicy, num_samples=100,
    ...
    replay_buffer = TFUniformReplayBuffer(
        data_spec=policy.trajectory_spec, batch_size=environment.batch_size, max_length=1000)
...
def behavioral_clone_original_policy_to_fsc(buffer: TFUniformReplayBuffer, num_epochs: int, agent: Recurrent_PPO_agent, memory_size: int) -> ClonedFSCActorPolicy:
    ...
    dataset = (
        buffer.as_dataset(sample_batch_size=64, num_steps=25, num_parallel_calls=tf.data.AUTOTUNE)
    ...
    def train_step(experience):
        ...
        with tf.GradientTape() as tape:
            played_action, mem = neural_fsc(observations, step_type=experience.step_type)
            loss = loss_fn(gt_actions, played_action)
            loss = tf.reduce_mean(loss)  # Přesný průměr přes batch
    ...
    for i in range(num_epochs):
        ...
        if i % 1000 == 0:
            ...
            evaluation_result = evaluate_policy_in_model(cloned_actor, agent.args, agent.environment, agent.tf_environment, 401, evaluation_result)


def run_experiment(prism_path, properties_path, memory_size, num_data_steps=100, num_training_steps=300):

    args = init_args(prism_path=prism_path, properties_path=properties_path, nr_runs=1)
    args.agent_name = "FSC_Clone"
    args.save_agent = True
    env, tf_env = init_environment(args)
    agent = Recurrent_PPO_agent(env, tf_env, args, agent_folder=args.agent_name, load=False)
    ...
    cloned_actor = behavioral_clone_original_policy_to_fsc(
        buffer, num_epochs=100000, agent=agent, memory_size=memory_size)
    
    return cloned_actor, buffer

if __name__ == "__main__":
    run_experiment("models_large/network-5-10-8/sketch.templ", "models_large/network-5-10-8/sketch.props", 2, 5000, 1000)
